...
function filterToolsForTask(task, tools) {
    if (tools.length === 0)
        return tools;
    ...
    for (const [serverName, serverTools] of byServer) {
        ...
        for (const token of taskTokens) {
            if (corpusTokens.has(token))
                score += 2; // exact match
            else if ([...corpusTokens].some((c) => c.includes(token) || token.includes(c)))
                score += 1; // partial
        }
    ...
    console.error(`[mcp-router] Filtered: kept [${chosen.join(", ")}], dropped [${dropped.join(", ")}] — ${filtered.length} tool(s) remaining`);
    return filtered;
}
// ---------------------------------------------------------------------------
...
async function runRoutingLoop(task, connector, config, client) {
    ...
    if (isAnthropic && !KNOWN_ANTHROPIC_MODELS.has(model)) {
        console.error(`[mcp-router] WARNING: Unrecognised model "${model}". Known models: ${[...KNOWN_ANTHROPIC_MODELS].join(", ")}`);
    }
...
async function recommendTool(task, tools, config, client) {
...
async function selectTool(task, toolIdMap, config, client) {
    const model = config.provider?.model ?? config.claude?.model ?? "claude-haiku-4-5-20251001";
    const maxTokens = config.provider?.maxTokens ?? config.claude?.maxTokens ?? 1024;
...
export function createDelegatedServer(connector, config, client) {
    const server = new Server({ name: "mcp-router", version: "1.0.0" }, { capabilities: { tools: {} } });
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: "select",
                description: "Describe a task in plain English. Returns the best matching tool name, its full input schema, suggested arguments, and a one-sentence reason — so you can review and then call execute().",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            {
                name: "execute",
                description: "Execute a downstream tool by its exact id (as returned by select). Pass the args object matching the tool's schema.",
                inputSchema: {
                    type: "object",
                    properties: {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        if (toolId === "execute") {
            const targetToolId = rawArgs.tool;
            if (!targetToolId) {
...
export function createSmartServer(connector, config, client) {
    const server = new Server({ name: "mcp-router", version: "1.0.0" }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: "recommend",
                description: "Describe a task in plain English. Claude picks the right tool, calls it, and returns the result directly.",
                inputSchema: {
                    type: "object",
                    properties: {
                        task: { type: "string", description: "Plain-English description of what you want to do" },
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        const tool = connector.tools.find((t) => `${sanitize(t.mcpName)}__${sanitize(t.name)}`.slice(0, 64) === toolId);
        if (!tool) {
        ...
        const result = await connector.callToolEntry(tool, (request.params.arguments ?? {}));
...
// MCP server factory
// ---------------------------------------------------------------------------
export function createRouterServer(connector, config, client) {
